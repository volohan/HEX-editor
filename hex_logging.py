            ...
            if log.new_byte == b'':
                if log.shift == 0:
                    if log.index - 1 in self.extended_bytes:
                        log = LogRecord(log.index - 1, self.extended_bytes[log.index - 1], log.new_byte, len(self.extended_bytes[log.index - 1]) - 1, log.is_insert)
                        self.extended_bytes[log.index][log.shift] = log.new_byte
                    else:
                        log = LogRecord(log.index - 1, None, log.new_byte, 0, log.is_insert)
                        if log.index == -1:
                            raise IndexError
                        self.extended_bytes[log.index] = bytearray()
                else:
                    log = LogRecord(log.index, self.extended_bytes[log.index], log.new_byte, log.shift - 1, log.is_insert)
                ...
                if len(self.extended_bytes[log.index]) == log.shift + 1:
                    if log.is_insert:
                        if log.index + 1 in self.extended_bytes:
                            ...
                            self.extended_bytes[log.index][log.shift] = log.new_byte[0]
                        else:
                            log = LogRecord(log.index + 1,
                                            None,
                                            log.new_byte, 0, log.is_insert)
                            self.extended_bytes[log.index] = bytearray(log.new_byte)
                    else:
                        log = LogRecord(log.index, self.extended_bytes[log.index], log.new_byte, log.shift + 1, log.is_insert)
                    ...
                    if log.is_insert:
                        ...
                        self.extended_bytes[log.index].insert(log.shift, log.new_byte[0])
            self.add(log)
        except IndexError:
            pass
